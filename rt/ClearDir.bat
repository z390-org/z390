@ECHO OFF
@ECHO.
@ECHO ClearDir - Remove all generated files by extension from directory %1

IF "%1" == "" (
	@ECHO.
	@ECHO ClearDir - Missing directory parameter!
	GOTO clrEnd
)

IF NOT EXIST %1\nul (
	@ECHO.
	@ECHO ClearDir - Cannot find given directory!
	GOTO clrEnd
)
