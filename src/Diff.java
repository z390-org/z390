 ...
 * The result of comparison is a list of <code>change</code> objects which form an edit script.
 * The objects compared are traditionally lines of text from two files.
 * Comparison options such as "ignore whitespace" are implemented by modifying the <code>equals</code>
 * and <code>hash code</code> methods for the objects compared.
 * <p>
 * The basic algorithm is described in:</br>
 * "An O(ND) Difference Algorithm and its Variations",</br>
 ...
 * <p>
 * You should have received a copy of the <a href=COPYING.txt>GNU General Public License</a> along
 * with this program; if not, write to the Free Software Foundation, Inc., 675 Mass Ave, Cambridge,
 * MA 02139, USA.
 */
public class Diff {

	/**
	 * Prepare to find differences between two arrays.  Each element of the arrays is translated
	 * to an "equivalence number" based on the result of <code>equals</code>.
	 * The original Object arrays are no longer needed for computing the differences.
	 * They will be needed again later to print the results of the comparison as an edit script, if desired.
	 */
	public Diff(Object[] a,Object[] b) {
		final Map<Object,Integer> h = new HashMap<Object,Integer>(a.length + b.length);
		filevec = new file_data[] { new file_data(a,h),new file_data(b,h) };
	}

	/**
	 * Prepare to find differences between two arrays.
	 * This constructor lets the caller computer equivalence numbers in any desired way.
	 * For instance, two char[] can be compared by copying to int[] with System.arraycopy().
	 */
	 ...
	 * With this heuristic, for files with a constant small density of changes, the algorithm is
	 * linear in the file size.
	 */
	public boolean heuristic = false;

	/**
	 * When set to true, the algorithm returns a guaranteed minimal set of changes.
	 ...
	 * breadth-first search through the space of edit-sequence.
	 * When the two searches meet, we have found the midpoint of the shortest edit sequence.
	 *
	 * The value returned is the number of the diagonal on which the midpoint lies.
	 * The diagonal number equals the number of inserted lines minus the number of deleted lines
	 * (counting only lines before the midpoint).
	 * The edit cost is stored into COST; this is the total number of lines inserted or deleted
	 * (counting only lines before the midpoint).
	 *
	 * This function assumes that the first lines of the specified portions of the two files do
	 * not match, and likewise that the last lines do not match.  The caller must trim matching
	 * lines from the beginning and end of the portions it is going to specify.
	 *
	 * Note that if we return the "wrong" diagonal value, or if the value of bdiag at that
	 * diagonal is "wrong", the worst this can do is cause sub-optimal diff output.
	...
	private int diag (int xoff, int xlim, int yoff, int ylim) {
		...
		/* True if south-east corner is on an odd diagonal with respect to the northwest. */
		...
		for (int c = 1;; ++c) {
			...
			/*
			 * Heuristic:
			 * Check occasionally for a diagonal that has made lots of progress compared with
			 * the edit distance. If we have any such, find the one that has made the most
	...
	/**
	 * Compare in detail contiguous subsequences of the two files which are known, as a whole,
	 * to match each other.
	 *
	...
	private void compareseq (int xoff, int xlim, int yoff, int ylim) {
		...
		//	int f = fdiag[fdiagoff + d];
			...
			if (c == 1) {
				// This should be impossible, because it implies that one of the two subsequences
				// is empty, and that case was handled above without calling 'diag'.
	 ...
	 * The script is described by a list of changes.
	 * The standard ScriptBuilder implementations provide for forward and reverse edit scripts.
	 * Alternate implementations could, for instance, list common elements instead of differences.
	 ...
	 * Each change represents one place where some lines are deleted and some are inserted.
	 *
	...
	class file_data {
		...
		void discard_confusing_lines(file_data f) {
			...
			/*	Don't really discard the provisional lines except when they occur
				in a run of discardables, with non-provisionals at the beginning
				and end. */
		...
		/**
		 * Don't really discard the provisional lines except when they occur in a run
		...
		/** Total number of non-discarded lines. */
