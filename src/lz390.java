 ...
 * to read OBJ relocatable object code files and generate single 390 load module file.<br>
...
public class lz390
{
	...
	private final static String msg_id_i = "LZ390I ";
	private final static String msg_id_e = "LZ390E ";
	private int lz390_rc = 0;
	private int lz390_errors = 0;
	private final static long tod_start = new Date().getTime();
	private int tot_obj_bytes = 0;
	private int tot_find_gbl_esd = 0;
	private boolean load_esds_only = true;
	private String lkd_entry = null;
	private int    lkd_entry_loc = -1;
	private String  obj_file_name = null;
	private boolean obj_file_bin  = false;
	private final static byte obj_bin_id   = 0x02;
	private RandomAccessFile obj_file = null;

	private int obj_end_entry_last_loc = 0; // RPI-1197 last .END entry load module offset

	private File           lst_file      = null;
	private BufferedWriter lst_file_buff = null;

	private final static int max_alias = 10;
	private String  alias_name[] = new String[max_alias];
	private int tot_alias = 0;
	private int tot_name  = 0;  // RPI-724

	private String obj_line = null;
	private JTextArea z390_log_text = null;

	//Global ESD tables
	private long    tod_time_limit = 0;
	private final static int next_time_ins   = 0x1000;
	private int     next_time_check = next_time_ins;
	private int tot_csect   = 0;
	private int tot_entry   = 0;
	private int tot_missing_wxtrn = 0;
	private int tot_gbl_esd = 0;
	private int cur_gbl_esd = 0;
	private int cur_gbl_ext = 0;
	private String[]  gbl_esd_name = null;
	private int[]     gbl_esd_loc  = null;
	private byte[]    gbl_esd_type = null;
	private final static byte gbl_esd_ext = 0; // undefined ext
	private final static byte gbl_esd_ent = 1; // found cst/ent
	private final static byte gbl_esd_wxt = 2; // undefined wxt
	private int loc_ctr = 0;
	private int mod_loc_ctr = 0;

	//Object files loaded
	private int  tot_obj_files = 0;
	private String[] obj_file_names = null;

	// Binary object file data
	private final static byte[] bin_byte = new byte[80];
	private ByteBuffer bin_byte_buff = ByteBuffer.wrap(bin_byte,0,80);

	// Current OBJ ESD table
	private int  tot_obj_esd      = 0;
	private int[]    obj_esd      = null;
	private String[] obj_esd_name = null;
	private int[]    obj_esd_loc  = null;
	private int[]    obj_esd_len  = null;
	private String[] obj_esd_type = null;

	// Current OBJ esd to gbl_esd_loc index table.
	private int[]    obj_gbl_esd = null;
	...
	//
	private final static String z390_code_ver = "1002";
	private String z390_flags = null;

	// Binary load module image in byte buffer
	private byte[]     z390_code      = null;
	private ByteBuffer z390_code_buff = null;
	...
	private int[]   rld_loc = null;
	private byte[]  rld_len = null;
	...
	/**
	 * Link one or more OBJ files into one single 390 load module.
	 *
	 * @param args
	 * @param log_text
	 * @return
	 */
	public int process_lz390(final String[] args, final JTextArea log_text)
	{
		/*
		 * Note this may be called directly from z390 GUI or from main when lz370 run
		 * from windows command line.
		 * If called from main, the log_text will be null and local put_log function
		 * will route to console instead of the z390 log window.
		 */
	 ...
	 */
	private void init_lz390(final String[] args, final JTextArea log_text)
	{
		/*
	...
	private void put_stats()
	{
		...
		put_log(msg_id_i +"total errors         = " + lz390_errors);
		tz390.force_nocon = false; // RPI-755
	}
	...
	private void log_error(final int error, final String msg)
	{
		...
		final String error_msg = msg_id_e + "error " + tz390.right_justify("" + error,3) + " " + msg;
		put_log(error_msg);
		tz390.put_systerm(error_msg);
		if (tz390.max_errors != 0 && lz390_errors > tz390.max_errors) {
			abort_error(5, "Maximum errors exceeded. Mamimum is "+ tz390.max_errors +" errors.");
		}
	}
	...
	private synchronized void abort_error(final int error, final String msg)
	{
		...
		if (tz390.z390_abort) {
			error_msg = msg_id_e + "abort due to recursive abort for " + msg;
		...
		error_msg = msg_id_e + "abort " + error + " " + msg;
		put_log(error_msg);
		tz390.put_systerm(error_msg);
		exit_lz390();
	}
	...
	private void put_copyright()
	{
		...
		put_log(msg_id_i + "program = " + tz390.dir_mlc + tz390.pgm_name + tz390.pgm_type);
		tz390.force_nocon = false; // RPI-755
	}
		...
		if (tz390.opt_list) {
			try {
				...
				if (lst_file.length() > tz390.max_file_size) {
					abort_error(34, "Maximum LST file size exceeded. Maximum is " + tz390.max_file_size + " bytes.");
				}
			} catch (final Exception e) {
				lz390_errors++;
			}
	...
	private void open_files()
	{
		...
		if (tz390.opt_list) {
			...
			try {
				...
				abort_error(9, "I/O error on LST file open - " + e.toString());
			}
		}
	}
	...
	private void process_lkd_cmds()
	{
		...
		final String lkd_file_name = tz390.find_file_name(tz390.dir_mlc, tz390.pgm_name, tz390.lkd_type, tz390.dir_cur);
		if (!tz390.lkd_ignore && lkd_file_name != null) {
			try {
				final RandomAccessFile lkd_file = new RandomAccessFile(lkd_file_name, "r");
				tz390.systerm_io++;
				String lkd_cmd = lkd_file.readLine();
				while (lkd_cmd != null) {
					...
					if (lkd_cmd.length() > 1 && lkd_cmd.charAt(0) != '*') {		
						...
						if (lkd_op.equals("INCLUDE")) { // INCLUDE
							...
							if (index1 > 0  && index2 > index1 + 1) {
								final String inc_ddname = lkd_parm.substring(0,index1);
								final String inc_pgm    = lkd_parm.substring(index1+1,index2);
								final String inc_path = System.getenv(inc_ddname);
								if (inc_path != null) {
	 ...
	 * @param pgm
	 */
	private void create_alias_390(final String alias, final String pgm)
	{
		try {
			final String alias_file_name = tz390.get_file_name(tz390.get_first_dir(tz390.dir_390),alias,tz390.z390_type);
			final File alias_file = new File(alias_file_name);
			final BufferedWriter alias_file_buff = new BufferedWriter(new FileWriter(alias_file));
	...
	/**
	 * Search and load OBJ files for externals until all found or no more can be resolved.
	 */
	 ...
	 */
	private boolean load_obj_file(final boolean esds_only)
	{
		open_obj_file(obj_file_name);
		if (!esds_only) {
			tz390.force_nocon = true;
			put_log(msg_id_i + "INCLUDE = " + obj_file_name); // RPI-1051
		...
		while (!obj_eod
			...
			if (obj_line.substring(0,4).equals(".END")) {
				...
				final int obj_end_entry_esd = Integer.valueOf(obj_line.substring( 9,13),16).intValue(); // RPI-1197 cur  .END entry esd or 0
				final int obj_end_entry_loc = Integer.valueOf(obj_line.substring(18,26),16).intValue(); // RPI-1197 cur  .END entry esd offset
				...
				if (!esds_only
					&& (obj_esd_type[tot_obj_esd].equals("CST")
					 ...
					 || obj_esd_type[tot_obj_esd].equals("WXT"))) { // RPI-182
				...
				if (!esds_only) {
					final int obj_text_esd = Integer.valueOf(obj_line.substring(9,13),16).intValue();
					...
					final int    obj_text_loc = Integer.valueOf(obj_line.substring(18,26),16).intValue();
					final int    obj_text_len = Integer.valueOf(obj_line.substring(31,33),16).intValue();
					final String obj_text = obj_line.substring(34,34 + 2 * obj_text_len);
				...
				if (!esds_only) {
					final int obj_rld_esd = Integer.valueOf(obj_line.substring(9,13),16).intValue();
					if (obj_rld_esd < 1 || obj_rld_esd > max_obj_esd) {
						abort_error(30, "Invalid rld esd - " + obj_rld_esd + " in " + obj_file_name);  // RPI-827
						return false;  // RPI-301
					}
					final int  obj_rld_loc = Integer.valueOf(obj_line.substring(18,26),16).intValue();
					final byte obj_rld_len = (byte) Integer.valueOf(obj_line.substring(31,32),16).intValue();
					final char obj_rld_sgn = obj_line.charAt(38);
					final int  obj_rld_xesd = Integer.valueOf(obj_line.substring(45,49),16).intValue();
					int rld_off = 0;
					int rld_fld = 0;
					if (tot_rld < tz390.opt_maxrld) {
						...
						if (gbl_esd_type[obj_gbl_esd[obj_rld_xesd]] == gbl_esd_ent) { //RPI-182
							...
							if (tz390.opt_list) {
								...
								put_lst_line(msg_id_i + "RLD LOC=" + tz390.get_hex(rld_off,8)
						...
						abort_error(21, "z390 rld table exceeded. Maximum is " + tz390.opt_maxrld + " entries.");
					}
				}
			} else {
		...
		try {
			...
			if (tot_obj_files < tz390.opt_maxfile) {
				...
				abort_error(18, "Maximum OBJ files exceeded. Maximum is " + tz390.opt_maxfile + " files.");
			}
			return true;
		} catch (final IOException e) {
	...
	private void add_gbl_esds()
	{
		...
		while (obj_index1 <= tot_obj_esd) {
			if (tot_gbl_esd >= tz390.opt_maxesd) {
				abort_error(15, "Maximum global ESDS exceeded. Maximum is " + tz390.opt_maxesd + " items.");
			}
	 ...
	 * @param obj_index1
	 */
	private void add_gbl_cst(final int obj_index1)
	{
		...
		if (esd_ok) {
			if (tz390.opt_list) {
				put_lst_line(msg_id_i +  "ESD=" + tz390.left_justify(obj_esd_name[obj_index1],8)
	 ...
	 * @param esd_type
	 */
	private void add_gbl_ref(final int obj_index1, final byte esd_type)
	{
		if (!find_gbl_esd(obj_esd_name[obj_index1])) {
			tot_gbl_esd++;
	 ...
	 * @param obj_index1
	 */
	private void add_gbl_ent(final int obj_index1)
	{
		tot_entry++;
		boolean esd_ok = true;
		if (find_gbl_esd(obj_esd_name[obj_index1])) {
	 ...
	 */
	private boolean open_obj_file(final String file)
	{
		try {
			obj_file = new RandomAccessFile(file,"r");
			tz390.systerm_io++;
			final int test_byte = obj_file.read();
	...
	private void gen_load_module()
	{
		...
		if (tz390.opt_mod && tot_rld > 0) {
			abort_error(40, "MOD file cannot contain RLDs = " + tot_rld + ".");
		}
		if (loc_ctr > tz390.max_file_size) {
			abort_error(32, "Maximum 390 file size exceeded. Maximum is " + tz390.max_file_size + " bytes.");
		}
		try {
			final String z390_sfx = tz390.opt_mod ? tz390.mod_type : tz390.z390_type;
			final RandomAccessFile z390_file = new RandomAccessFile(tz390.get_first_dir(tz390.dir_390) + tz390.pgm_name + z390_sfx, "rw"); // RPI-883
			...
			while (cur_rld < tot_rld) {
				...
				if (z390_file.length() > tz390.max_file_size) {
					abort_error(33, "Maximum 390 file size exceeded. Maximum is " + tz390.max_file_size + " bytes.");
				}
				cur_rld++;
			}
			z390_file.close();
