...
public  class  sz390 implements Runnable
{
	...
	int max_ecb_count     =    16;
	int max_env_name_len  =   256;
...
//	private SimpleDateFormat cur_date_MMddyy   = new SimpleDateFormat("MM/dd/yy"); // JBA - unused
//	private SimpleDateFormat cur_tod_hhmmss    = new SimpleDateFormat("HH:mm:ss"); // JBA - unused
	private SimpleDateFormat cur_tod_hhmmss00  = new SimpleDateFormat("HHmmss00");
	private SimpleDateFormat cur_date_yyyy     = new SimpleDateFormat("yyyy");
	private SimpleDateFormat cur_date_MM       = new SimpleDateFormat("MM");
	private SimpleDateFormat cur_date_dd       = new SimpleDateFormat("dd");
	private SimpleDateFormat cur_date_HH       = new SimpleDateFormat("HH");
	private SimpleDateFormat cur_date_mm       = new SimpleDateFormat("mm");
	private SimpleDateFormat cur_date_ss       = new SimpleDateFormat("ss");
	private SimpleDateFormat cur_date_ms       = new SimpleDateFormat("SSS");
	private SimpleDateFormat cur_date_yyddd    = new SimpleDateFormat("yyDDD");
	private SimpleDateFormat cur_date_yyyyddd  = new SimpleDateFormat("yyyyDDD");
	private SimpleDateFormat cur_date_MMddyyyy = new SimpleDateFormat("MMddyyyy");
	private SimpleDateFormat cur_date_ddMMyyyy = new SimpleDateFormat("ddMMyyyy");
	private SimpleDateFormat cur_date_yyyyMMdd = new SimpleDateFormat("yyyyMMdd");
	int  cur_date_year  = 0;
	...
	//	(negative len means subtract the base address versus adding it to rld field.)
	int  rld_loc = 0;
	byte rld_len = 0;
	...
	public void svc(final int svc_id)
	{
		...
		case 0x79: // VSAM access method
			vz390.cur_vsam_op = _pz390.reg.get(pz390.r0 + 3);
	...
	{
		final int r15_rc = _pz390.reg.getInt(pz390.r15); //RPI39
		if (r15_rc > ez390_rc) {
			ez390_rc = r15_rc;
		}
	...
	/**
	 * Init pz390.cur_date and calendar with current time and date or force
	 * fixed time if NOTIMING option set.
	 * Notes:
	 *   1.  This NOTTIMING option is used in regression testing timing functions
	 ...
	 * 2. Initialize optional test=ddname file for batch input else init test_cmd_file
 ...
 private void svc_exit(){
	...
	if (stimer_exit_running) {
		...
		int estae_restart_psw = _pz390.reg.getInt(pz390.r0);
		int estae_exit_rc = _pz390.reg.getInt(pz390.r15);
 	...
 	if (tot_link_stk == 1) {
 		_pz390.set_psw_check(_pz390.psw_pic_exit);  // exit ez390
 	} else {
 		...
 		if ((link_ret & _pz390.psw_amode31_bit) != 0){
 			_pz390.set_psw_amode(_pz390.psw_amode31_bit);
 		} else {
 			_pz390.set_psw_amode(_pz390.psw_amode24_bit);
 		}
 	}
 }
private void svc_extract(){ // RPI 413
	 ...
	 */
	int op = _pz390.reg.getInt(pz390.r0);
	switch (op){
	case 1: // GETENV r1=name, r2 set to getmain'd value with null term
        ...
        break;
	default:
		_pz390.set_psw_check(_pz390.psw_pic_spec);
	}
}
...
private void svc_load_390(){
  ...
  try {
    ...
    if (z390_flags[z390_flags_amode31] == 'T'){
 	    load_code_ent  = load_code_ent  | _pz390.psw_amode31_bit;
        load_code_load = load_code_load | _pz390.psw_amode31_bit;
    }
    add_cde();
    svc_load_set_regs();
  } catch (final Exception e) {
...
private void svc_load_file(){
  ...
  try {
    ...
    z390_file.seek(0); 
    if (_pz390.psw_amode == _pz390.psw_amode31){  // load file above line if amode31
    ...
    if (_pz390.reg.getInt(pz390.r15) == 0){
       ...
       tz390.systerm_io++;
       z390_file.read(_pz390.mem_byte,load_code_load,load_code_len);
       if (_pz390.psw_amode == _pz390.psw_amode31){
           load_code_load = load_code_load | _pz390.psw_amode31_bit;
       }
    } else {
    	abort_error(15,"getmain for svc 8 load failed - PGM=" + tz390.pgm_name + "LEN=" + load_code_len);
...
private void svc_xctl(){
	 ...
	 *   2. abend s106
	 */
	int user_parm = _pz390.reg.getInt(pz390.r1); // RPI 596 RPI 598
	svc_load();
	if (_pz390.reg.getInt(pz390.r15) != 0){
		if (_pz390.tot_estae == 0){
			log_error(102,"xctl program not found - " + load_pgm_name);
		}
		_pz390.set_psw_check(_pz390.psw_pic_link_err); // link failed
		return;
	}
	int link_addr = _pz390.reg.getInt(pz390.r0);
	if  (_pz390.reg.getInt(pz390.r15) == 0) {
        ...
        if ((link_addr & _pz390.psw_amode31_bit) != 0){
            _pz390.set_psw_amode(_pz390.psw_amode31_bit);
        } else {
            _pz390.set_psw_amode(_pz390.psw_amode24_bit);
        }
...
private boolean get_eploc_pgm_name(){
	 ...
	 */
		int ep_loc = _pz390.reg.getInt(pz390.r0) & _pz390.psw_amode;
 		if (ep_loc != 0){
...
public void svc_getmain(){
	...
	if (req_len <= 0){
		_pz390.set_psw_check(_pz390.psw_pic_gm_err);
		return;
	}
	...
	if ((req_opt & opt_getmain_cond) == 0){
		_pz390.set_psw_check(_pz390.psw_pic_no_mem);
	}
}
...
public void svc_freemain(){
	 ...
	 */
	req_addr = _pz390.reg.getInt(pz390.r1) & _pz390.psw_amode31; // RPI 244
	req_len = _pz390.reg.getInt(pz390.r0);	// RPI 244
	req_len = (req_len + 7)/8*8; // round to 8
	if (req_addr < _pz390.dsa24_end) {
		if (   req_len <= 0
			...
			_pz390.set_psw_check(_pz390.psw_pic_fm_err);
			return;
		}
		cur_fqe = _pz390.mem.getInt(pz390.zcvt_fqe24);
		prev_fqe = pz390.zcvt_fqe24;
	...
	while (cur_fqe > 0) {
		...
		if (req_addr < cur_fqe) {
            ...
            if (cur_fqe + cur_fqe_len == next_fqe){
    			...
    			_pz390.set_psw_check(_pz390.psw_pic_bad_mem);
    			return;
            } else {
            	_pz390.mem.putInt(cur_fqe+4,cur_fqe_len);
            }
...
private void svc_time(){
	 ...
	 */
	int date_type = _pz390.reg.getShort(pz390.r0);
	int time_type = _pz390.reg.getShort(pz390.r0+2);
	int time_addr = _pz390.reg.getInt(pz390.r1) & _pz390.psw_amode;
	if (tz390.opt_timing){
	    _pz390.cur_date = new Date();
	}
...
private void svc_ttimer(){
	 ...
	 */
	int  opcode = _pz390.reg.getInt(pz390.r0);
...
private void svc_stimer() {
	 ...
	 */
	byte req_type   = _pz390.reg.get(0);
	byte intvl_type = _pz390.reg.get(1);
	int  intvl_addr = _pz390.reg.getInt(pz390.r1) & _pz390.psw_amode;
	long intvl_mics = 0;
	/*
...
;		break;
	case 2: // DINTVL=RX DWORD PL8'HHMMSSTH'
		long hhmmssth = _pz390.mem.getLong(intvl_addr) >>> 4;
...
private int get_ccyydddf(){
	 ...
	 */
    int cc = Integer.valueOf(cur_date_yyyy.format(_pz390.cur_date))/100-19;
    return (cc << 24) 
...
private void svc_bldl(){
	 ...
	 * else 0.
	 */
	int bldl_list_addr = _pz390.reg.getInt(pz390.r1) & _pz390.psw_amode;
	...
	while (bldl_list_count > 0){
		bldl_entry_len = _pz390.mem.getShort(bldl_entry_addr);
...
public void svc_abend(int pic, boolean type, boolean req_dump){
	...
	int save_psw_loc = _pz390.psw_loc; // RPI 1054
...
private void list_trace_table(){
	 ...
	 */
	int cur_index = _pz390.trace_table_index;
	int index = _pz390.trace_table_next[cur_index];
	while (index != cur_index){
		// list last 10 if any
		int addr = _pz390.trace_table_addr[index];
		if (addr != 0){
...
private String dump_psw(){ // RPI 819
	 ...
	 */
	int   cc    = _pz390.psw_cc_code[_pz390.psw_cc];
	int   mask  = _pz390.psw_pgm_mask;
	int   amode_and_addr;
	if (_pz390.psw_amode == _pz390.psw_amode31){
		amode_and_addr = _pz390.int_high_bit
		        | _pz390.psw_loc;
	} else {
...
public void dump_fpr(int reg_offset){
	...
	if (reg_offset < 0 || reg_offset > pz390.r15){
		int reg_off = 0;
		while (reg_off < _pz390.max_reg_off){  // RPI 229
...
private void dump_mem_stat(){
	 ...
	 * totals on log
	 */
	tz390.put_trace(" MEM TOTAL=" + (_pz390.tot_mem >> 20) + "MB  ALLOC=" + _pz390.tot_mem_alloc 
...
private String get_dcb_file_name(String dsnam_path){
	 ...
	 */
	String file_name = "";
	int dcb_dsn = _pz390.mem.getInt(cur_dcb_addr + dcb_dsnam);
	if (dcb_dsn > 0){
...
private void dcb_synad_error(int error_num,String error_msg){
	...
	if (cur_dcb_synad == 0){
		...
		_pz390.set_psw_check(_pz390.psw_pic_io); //RPI64
	} else {
...
public String get_ascii_string(int mem_addr,int mem_len,boolean null_term){
	...
	while (index < mem_len
			&& (!null_term  // RPI 685 allow nulls in ASCII text records
				|| _pz390.mem_byte[mem_addr + index] != 0
				)){  // RPI 642
		if (tz390.opt_ascii){
...
public void put_ascii_string(String text,int mem_addr,int mem_len,char pad_char){
	...
	char text_char;
	int index = 0;
	mem_addr = mem_addr & _pz390.psw_amode;  // RPI 712
...
private void svc_cmd(){
	 ...
	 */
	String svc_cmd_text = "";
	int cmd_id = _pz390.reg.get(pz390.r0+2);
	int cur_cmd_op = _pz390.reg.get(pz390.r0+3);
	switch (cur_cmd_op){
	...
	case 3: 
		...
		try {
			...
			if  (cmd_read_line != null){
				put_ascii_string(cmd_read_line[cmd_id],_pz390.reg.getInt(pz390.r1),_pz390.reg.getInt(pz390.r2),' ');
				_pz390.reg.putInt(pz390.r15,0);
...
private int cmd_startup(int cmd_id){
	/*
	 * start Windows command processor with 
...
private synchronized void svc_tget_tput(){ // RPI 318
	...
	int buff_len   = _pz390.reg.getShort(pz390.r0+2);
	int buff_addr  = _pz390.reg.getInt(pz390.r1) & _pz390.psw_amode24;
	String wto_msg = null;
	if (tz390.opt_guam){
		if ((tpg_flags & tpg_op_mask) == tpg_op_tput){
			...
			gz390.tput_buff.put(_pz390.mem_byte,buff_addr,buff_len);
			...
			if (gz390.tpg_rc == 0){  // RPI 712
				...
				_pz390.mem.put(gz390.tget_byte,0,gz390.tget_len);
				if (tz390.opt_tracet){  // RPI 671
					tz390.put_trace("");
					tz390.put_trace(" TGET bytes received = " + tz390.get_hex(gz390.tget_len,4));
					dump_mem(_pz390.mem,buff_addr,gz390.tget_len);
					tz390.put_trace("");
				}
				_pz390.reg.putInt(pz390.r1,gz390.tget_len);
			}
		}
		_pz390.reg.putInt(pz390.r15,gz390.tpg_rc); // RPI 221 set retrun code
	} else {
...
private void svc_snap(){
	 ...
	 */
	int text_addr = _pz390.reg.getInt(pz390.r1);
	if (text_addr > 0){
	...
	int flags = _pz390.reg.getShort(pz390.r0);
	if ((flags & 0x4000) != 0){
		dump_gpr(-1);
	}
	if ((flags & 0x2000) != 0){
	...
	if ((flags & 0x8000) != 0){
		int dump_addr =  _pz390.reg.getInt(pz390.r14) & _pz390.psw_amode;
		int dump_len  = (_pz390.reg.getInt(pz390.r15) & _pz390.psw_amode) - dump_addr;
	...
	while (ecb_count > 0){
		...
		if (ecb_addr < 0){
			if (list_entry_count < wait_count){ // RPI 393
				_pz390.set_psw_check(_pz390.psw_pic_waiterr);
			}
			return false;
		}
...
private void reset_wait_list(){
	...
	while (ecb_addr != 0 && ecb_list_count > 0){
		...
		int ecb_code = _pz390.mem.getInt(ecb_addr & 0x7fffffff);
...
private void svc_post(){  // RPI 279
	 ...
	 * post ecb complete
	 */
	int ecb_addr = _pz390.reg.getInt(pz390.r1) & _pz390.psw_amode;
	int ecb_code = _pz390.reg.getInt(pz390.r0) & _pz390.max_pos_int;
...
private void svc_espie(){
	 ...
	 */
	int espie_pie   = _pz390.reg.getInt(pz390.r0 ); // save pz390.psw_pic bit mask
	int espie_addr  = _pz390.reg.getInt(pz390.r1 ) & _pz390.psw_amode; // save exit psw addr
	int espie_param = _pz390.reg.getInt(pz390.r15) & _pz390.psw_amode; // save exit psw addr
	if (espie_addr == 0){
		if (_pz390.tot_espie > 0){
			_pz390.tot_espie--;
		}
		_pz390.reg.putInt(pz390.r15,0);
	} else if (_pz390.tot_espie < _pz390.max_espie){
		if (espie_addr < 0){
			if (_pz390.tot_espie == 0){
				_pz390.tot_espie++;
			}
		...
		_pz390.set_psw_check(_pz390.psw_pic_error);
	}
}
private void svc_estae(){
	...
	int estae_param = _pz390.reg.getInt(pz390.r1) & _pz390.psw_amode;
	if (estae_addr == 0){
		if (_pz390.tot_estae > 0){
			_pz390.tot_estae--;
		}
		_pz390.reg.putInt(pz390.r15,0);
	} else if (_pz390.tot_estae < _pz390.max_estae){
		if (estae_addr < 0){
			if (_pz390.tot_estae == 0){
				_pz390.tot_estae++;
			}
...
private void svc_ctd(){
	...
	case 6: // lh 
		if (addr_in >= 16){ // RPI 507
			...
			if (!_pz390.fp_pair_valid[addr_in]){ // RPI 512
				_pz390.reg.putInt(pz390.r15,8);
			    return;
			}
		 	if (_pz390.fp_reg_ctl[addr_in] != _pz390.fp_ctl_ld){
	...
	while (index < ctd_display_len){
		if (tz390.opt_ascii){ // RPI 477
			_pz390.mem_byte[addr_out] = (byte)' ';
		} else {
			_pz390.mem.put(addr_out,(byte)ebcdic_space);
		}
		index++;
		addr_out++;
	}
...
private void svc_cfd(){
	...
	int addr = _pz390.reg.getInt(pz390.r1) & _pz390.psw_amode;
	byte type = _pz390.mem.get(addr+3);
	int addr_out  = _pz390.mem.getInt(addr+4) & _pz390.psw_amode; // RPI 526
	int addr_in = _pz390.mem.getInt(addr+8) & _pz390.psw_amode;   // RPI 526
	String cfd_text = tz390.get_ascii_var_string(_pz390.mem_byte,addr_in,ctd_display_len).trim();  
	switch (type){
	...
	case 26: // lh 
		...
		if (addr_out >= 16){  // RPI 507
			...
			if (_pz390.fp_pair_valid[addr_out]){ // RPI 512
...
private void exec_test_cmd(){
  ...
  try { // RPI 1137	
	...
	case 'Z':  // zoom to normal end by turning off trace and test
        ...
        _pz390.test_trace_count = -1; //RPI186
	...
	while (test_sign == '+' || test_sign == '-'){	
	    ...
	    while (test_token != null 
	    	...
	    	if (test_token.charAt(0) == '?'){
	    		total = _pz390.mem.getInt(total & _pz390.psw_amode31);
	    	} else {
	    		total = _pz390.mem.getInt(total & _pz390.psw_amode24);
	    	}
	    	test_token = get_next_test_token();
	    }
	    if (test_token == null){
...
private void set_test_break_op(){
	...
	if (index != -1){
		...
		test_break_op2_index = _pz390.opcode2_offset[test_break_op1];
		if (tz390.op_code[index].length() == 4){
			...
			if (_pz390.opcode2_mask[test_break_op1] == 0xf0){
			    try {
	...
	try {
		if (text.length() > 1){
			if (text.toUpperCase().charAt(text.length()-1) == 'R'){
				...
				addr = (_pz390.reg.getInt(get_test_int(text.substring(0,index_r))*8+4)) & _pz390.psw_amode24;
				while (text.charAt(index_p) == '%'){
				    addr = _pz390.mem.getInt(addr) & _pz390.psw_amode24;
				    index_p--;
				}
			} else if (text.toUpperCase().charAt(text.length()-1) == '?'){
				int index_r = text.toUpperCase().indexOf('R');
				int index_q = text.length()-2;
				addr = (_pz390.reg.getInt(get_test_int(text.substring(0,index_r))*8+4)) & _pz390.psw_amode31;
				while (text.charAt(index_q) == '?'){
				    addr = _pz390.mem.getInt(addr) & _pz390.psw_amode31;
				    index_q--;
				}
...
private void svc_tcpio(){
	...
	case 4: // send message
		...
		if (tcpio_find_client_port()){
			...
			int conn_index = _pz390.reg.getInt(pz390.r2);
			if (conn_index >= 0 
				&& conn_index < max_tcp_conn
 ...
 */ 	
 	...
 	public void ast_close_file(int tiot_index){
        ...
        try {
        	...
        	dcb_synad_error(26,"i/o error on close - " + e.toString());
        	_pz390.set_psw_check(_pz390.psw_pic_io);
        }
 	}
 	...
 	private void zsort_init_parms(){
 		 ...
 		 */
            ...
            while (index < zsort_max_keys && zsort_tot_keys == 0){
            	...
            	if (_pz390.mem.get(zsort_parm_addr + 8) != 0){ // check for VL bit on last key full word parm
 	...
 	private void zsort_put(){
		...
		zsort_move_rec(_pz390.reg.getInt(pz390.r1), zsort_blk_ptr); // move rec
																	// to blk
		if (tz390.opt_traceall) {
 		...
 		if (!zsort_get){
			...
			if (zsort_sortwk_len > 0){
				if (zsort_blk1_ptr < zsort_blk1_ptr_end){
					zsort_move_rec(zsort_blk1_ptr,_pz390.reg.getInt(pz390.r1)); // move from single blk
				...
				if (zsort_blk_ptr < zsort_blk_end){
					zsort_move_rec(zsort_blk_ptr,_pz390.reg.getInt(pz390.r1)); // move from singel blk
					zsort_blk_ptr = zsort_blk_ptr + zsort_lrecl;
				} else {
					// return rc 4 end of file
					zsort_term();
			...
			int index1 = rec1 + zsort_key_off[key_index];
 			...
 			case 7: // ZD - zoned decimal
				...
				if (int2 == 0xd0){
 				...
 				byte save_sign1 = _pz390.mem_byte[index1 + key_len - 1];
 				byte save_sign2 = _pz390.mem_byte[index2 + key_len - 1];
