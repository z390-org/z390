...
public class vz390
{
	...
	int ksir_fwd = 36; // next forward KSIR or -1
	...
	public void svc_vsam()
	{
		...
		case 20: // CLOSE R1=A(ACB)
			...
			break;
		default:
			_pz390.set_psw_check(_pz390.psw_pic_oper);
		}
	}
	...
	private boolean find_vclr()
	{
		...
		cur_vcdt_clrt = _pz390.mem.getInt(cur_vcdt_addr + vcdt_clrt);
		...
		cur_vcdt_ptht = _pz390.mem.getInt(cur_vcdt_addr + vcdt_ptht);  // RPI 965 
		...
		while (cur_vcdt_ptht > 0) {
			if (sz390.load_vcdt_entry.equals(sz390.get_ascii_string(
				...
				cur_vpth_flag = _pz390.mem.getInt(cur_vcdt_ptha + vpth_flag); // RPI 865 
	...
	private void fetch_rpl_fields()
	{
		...
		cur_rpl_ecb  = _pz390.mem.getInt(cur_rpl_addr + rpl_ecb);
		cur_rpl_lkey = _pz390.mem.getInt(cur_rpl_addr + rpl_lkey);
		...
		cur_rpl_next = _pz390.mem.getInt(cur_rpl_addr + rpl_next);
		cur_rpl_lrec = _pz390.mem.getInt(cur_rpl_addr + rpl_lrec);
		if (cur_rpl_lrec == 0) {
			cur_rpl_lrec = cur_vclr_lrec; // RPI 750
		}
		cur_rpl_larea = _pz390.mem.getInt(cur_rpl_addr + rpl_larea);
		cur_rpl_lxrba = _pz390.mem.getLong(cur_rpl_addr + rpl_lxrba);	// last
																		// rec
																		// RPI
																		// 702
		cur_rpl_cxrba = _pz390.mem.getLong(cur_rpl_addr + rpl_cxrba);	// curr
																		// pos
																		// RPI
																		// 702
		cur_rpl_openc = _pz390.mem.getInt(cur_rpl_addr + rpl_openc);
		cur_rpl_flag = _pz390.mem.getInt(cur_rpl_addr + rpl_flag);	// last rec
																	// RPI 702
		cur_rpl_ksit = _pz390.mem.getLong(cur_rpl_addr + rpl_ksit);	// curr pos
																	// RPI 702
	...
	private boolean open_acb_dcbs()
	{
		...
		cur_vclr_vesa = _pz390.mem.getInt(cur_acb_vclra + vclr_vesa);
		...
		if (cur_acb_dcbt > 1) {
			if (cur_vcdt_ptha != 0 && cur_vaix_addr != 0){  // RPI 865  
				...
				cur_vclr_vx0a = _pz390.mem.getInt(cur_acb_vclra + vclr_vx0a);
			...
			while (index < cur_acb_dcbt) {
				...
				cur_vaix_vxna = _pz390.mem.getInt(cur_vaix_addr + vaix_vxna);
				cur_vaix_klen = _pz390.mem.getInt(cur_vaix_addr + vaix_klen);
	...
	private boolean get_rrds_ves_xrba(boolean key) {
		 ...
		 */
		if (key) {
			long rrn = (long) _pz390.mem.getInt(cur_rpl_arg); // RPI 724
			if (rrn == -1) {
	...
	private void svc_rpl_point() {
		...
		if ((cur_vclr_flag & vclr_flag_esds) != 0) {
			...
			// RRDS\VRRDS
			cur_ves_rec = _pz390.mem.getInt(cur_rpl_arg);
			if ((cur_rpl_opt & rpl_opt_lrd) != 0) {
				// position to eof xrba
				cur_vx0_xrba = sz390.tiot_eof_rba[cur_vx0_tiot_index];
			} else {
	...
	private void set_feedback(byte pdf, byte rc, byte cmp, byte rn) {
		...
		if (tz390.opt_tracev) {
			...
			sz390.dump_mem(_pz390.mem, cur_rpl_arg, cur_rpl_arg_len); // RPI
																		// 779
	...
	private boolean read_ves_rec() {
		...
		if ((cur_vclr_flag & vclr_flag_vrec) != 0) {
			...
			try {
				...
				_pz390.mem.putInt(cur_rpl_addr + rpl_lrec, cur_rpl_lrec); // RPI
																			// 688
				if (get_vcb_buff(cur_ves_tiot_index, cur_ves_xrba + 4,
						cur_rpl_lrec)) {
					tot_ves_cache++;
		...
		try {
			...
			if (tz390.opt_tracev) {
				tz390.put_trace("VSAM EXCP WRITE KSDS  INDEX KEY XRBA="
						+ tz390.get_long_hex(cur_vx0_xrba + 8, 16));
				sz390.dump_mem(_pz390.mem, cur_rpl_area + cur_vclr_koff,
						cur_vclr_klen);
			}
			cur_vx0_xrba = cur_vx0_xrba + 8 + cur_vclr_klen;
			sz390.tot_dcb_write++;
	...
	private boolean reuse_file(int adcb) {
		 ...
		 */
		int tiot_index = _pz390.mem.getInt(adcb + sz390.dcb_iobad) - 1;
		try {
