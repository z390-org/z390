...
public class zc390
{
	...
	*/
	...
	Matcher zc_name_match    = null;   // data name pattern matching
	...
	/**
	 * Display zc390 version, timestamp, and copyright on statistcs file.
	 */
		...
		if (zc_token.length() > 1) {
			...
			if (zc_token.charAt(0) == '.') {
				if (mlc_op != null
					...
					// flush line at period after parameters
			...
			if (zc_proc_div 
				...
				if  (zc_token.charAt(0) >= '0' 
					...
					if (zc_token.equals("DATA")){
						...
						if (zc_cics){
							// check if PROCEDURE statement needs to be preceeded by generated 
	 ...
	 * 2.  Add any user parmeters after above.
	 ...
	 * 2.  Generate new WS verb with level # as first parameter indented to current group level.
	 */
	...
	private void add_cpz_file()
	{
		...
		int index = tz390.find_key_index('F',zc_copy_file_name);
		if (index == -1) {
